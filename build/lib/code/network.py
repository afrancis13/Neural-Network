...
class NeuralNetwork(object):
    ...
    def train(self):
        ...
        while not converged:
            if training_error is not None and training_loss is not None:
                training_error_array.append(training_error[0])
    ...
    def predict(self, X, V, W, classify=True):
        '''
        ...
        if classify:
            ...
            return classifications.reshape(len(classifications), 1)
        else:
            return Z
    ...
    def converged_gradient(self, num_iter, X, V, W, iter_check=20000, threshold=0.01,
        ...
        if error:
            if num_iter % iter_check != 0:
                ...
                classifications_training = self.predict(X, V, W)
                ...
                if self.loss_function == "mean-squared-error":
                    training_loss = mean_squared_error(classifications_training, self.labels)
                else:
                    training_loss = cross_entropy_loss(classifications_training, self.labels)
