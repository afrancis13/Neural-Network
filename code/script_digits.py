    ...
    for epsilon in learning_rates:
        ...
        for cv_set in cv_sets:
            ...
            if RUN_MSE:
                ...
                trained_V_mse, trained_W_mse, train_accuracy_array_mse, train_loss_array_mse = classifier_mse.train()
                ...
                # Save state.
                file_V_mse_name = "../pickle/V_matrix_mse%d.txt" % file_number
                file_W_mse_name = "../pickle/W_matrix_mse%d.txt" % file_number
                ...
                if PLOTS:
                    plt.plot(range(len(train_accuracy_array_mse)), train_accuracy_array_mse)
            ...
            if RUN_CROSS_ENTROPY:
                ...
                trained_V_cross_entropy, trained_W_cross_entropy, train_accuracy_array_cross_entropy, train_loss_array_cross_entropy = classifier_cross_entropy.train()
                ...
                # Save state.
                pickle.dump(trained_V_cross_entropy, open("../pickle/V_matrix_cross_entropy%d.txt" % file_number, "wb"))
                pickle.dump(trained_W_cross_entropy, open("../pickle/W_matrix_cross_entropy%d.txt" % file_number, "wb"))

                print("Saved matrices to files on the local machine.")
                if PLOTS:
                    plt.plot(range(len(train_accuracy_array_cross_entropy)), train_accuracy_array_cross_entropy)
                    plt.title("Training Error vs. Number of Iterations")
                    plt.ylabel("Classification Error (%)")
                    plt.xlabel("Number of Iterations")
                    plt.show()
...
if RUN_KAGGLE:
    ...
    print("The total training time for the Neural Network classifier is %.4f" % post_training_time - pre_training_time)
